#ifndef __FORWARD_BACKWARD_R_H
#define __FORWARD_BACKWARD_R_H


void markov_chain::combine_prob( vector<int> &position, map<int, vector<vector<int> > > &states,
                                 vector<string> &chrom,
                                 bool output_pulses,
                                 vector<pulse> &pulses,
  ...
  /// output ancestry pulses rather than ancestry states [default]
    ...
    // header printing, switched off for R
    /*
    ...
    for ( int i = 0 ; i < alphas.size() ; i ++ ) {
      ...
      //out << chrom.at(i) << "\t" << position.at(i) ;
      //  Rcpp::Rcout << "chrom and pos\n";
      //  Rcpp::Rcout << chrom.at(i) << "\t" << position.at(i) << "\n"; force_output();
      snp.push_back(static_cast<double>(std::stoi(chrom.at(i))));
      snp.push_back(static_cast<double>(position.at(i)));
      for ( int l = 0 ; l < smoothed_probs.n_rows ; l ++ ) {
        //out << "\t" << smoothed_probs(l) ;
        snp.push_back(smoothed_probs(l));
      }
      output.push_back(snp);
      //out << endl ;
    }
  }
  // out.close() ;
